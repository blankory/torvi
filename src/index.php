...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $scheduled_time = $datetime_start = $datetime_end = $url = "";

    // preprocess form info
    if (isset($_POST["url"])) {
        $url = $_POST["url"];
    }
    if (isset($_POST["scheduler_check"])) {
        $scheduleMessage = filter_var($_POST["scheduler_check"], FILTER_VALIDATE_BOOLEAN); // convert string to boolean
        $scheduled_time = $_POST["scheduler_time"];
    }
